...
pub async fn get_courses_for_tutor_db(
    ...
    let course_rows = sqlx::query_as!(
        Course,
        "select * from tutor_course where tutor_id = $1",
        tutor_id
    )
    .fetch_all(pool)
    .await?;
...
pub async fn get_course_details_db(
    ...
    let course_row = sqlx::query_as!(
        Course,
        "select * from tutor_course where tutor_id = $1 and course_id = $2",
        tutor_id,
        course_id
    )
    .fetch_optional(pool)
...
pub async fn delete_course_db(
    ...
    tutor_id: i32,
    course_id: i32,
) -> Result<String, TutorFrontError> {
    let course_row = sqlx::query!(
        "delete from tutor_course where tutor_id = $1 and course_id = $2",
        tutor_id,
        course_id
    )
    .execute(pool)
...
pub async fn update_course_details_db(
    ...
    .fetch_one(pool)
    .await
    .map_err(|_err| TutorFrontError::NotFound("Course id not found".into()))?;
    ...
    let course_row = sqlx::query_as!(
        Course,
        "UPDATE tutor_course set course_name = $1,
