...
pub async fn get_courses_for_tutor(
    ...
    path: web::Path<i32>,
) -> Result<HttpResponse, TutorFrontError> {
...
mod tests {
    ...
    #[actix_rt::test]
    async fn get_courses_detail_success_test() {
        dotenv().ok();
        let database_url = env::var("DATABASE_URL").expect("DATABASE_URL is not set in .env file");
        let pool = PgPool::connect(&database_url).await.unwrap();
        let app_state = web::Data::new(AppState {
    ...
    #[actix_rt::test]
    async fn get_courses_detail_failure_test() {
        dotenv().ok();
        let database_url = env::var("DATABASE_URL").expect("DATABASE_URL is not set in .env file");
        let pool = PgPool::connect(&database_url).await.unwrap();
        let app_state = web::Data::new(AppState {
    ...
    #[ignore = "no need to create new test case"]
    #[actix_rt::test]
